...
public class TupleValueMapTest extends TupleCollectionTest {
	...
	public void testValueMapCRUID() {
		...
		Map<Integer, Section> actualMap = session.select(book::contents).where(book::id, Query.eq(id)).sync().findFirst().get()._1;
		assertEqualMaps(contents, actualMap);

		// read single key-value in map

		String cql = session.select(Query.get(book::contents, 1))
