...
public class TupleKeyMapTest extends TupleCollectionTest {
	...
	public void testKeyMapCRUID() {
		...
		Map<Section, String> actualMap = session.select(book::notes).where(book::id, Query.eq(id)).sync().findFirst().get()._1;
		assertEqualMaps(notes, actualMap);

		// read single key-value in map

		String cql = session.select(Query.get(book::notes, new SectionImpl("first", 1)))
