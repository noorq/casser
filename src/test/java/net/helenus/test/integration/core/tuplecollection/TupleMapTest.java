...
import net.helenus.test.integration.core.udtcollection.UDTCollectionTest;
...
public class TupleMapTest extends TupleCollectionTest {
	...
	public void testMapCRUID() {
		...
		Map<Section, Author> actualMap = session.select(book::writers).where(book::id, Query.eq(id)).sync().findFirst().get()._1;
		assertEqualMaps(writers, actualMap);

		// read single key-value in map

		String cql = session.select(Query.get(book::writers, new SectionImpl("first", 1)))
