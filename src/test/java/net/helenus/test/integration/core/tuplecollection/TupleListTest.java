...
public class TupleListTest extends TupleCollectionTest {
	...
	public void testListCRUID() {
		...
		List<Author> actualList = session.select(book::authors).where(book::id, Query.eq(id)).sync().findFirst().get()._1;
		assertEqualLists(authors, actualList);

		// read single value by index

		String cql = session.select(Query.getIdx(book::authors, 1))
				.where(book::id, Query.eq(id)).cql();
		...
		session.update().prepend(book::authors, new AuthorImpl("Prepend", "PrependCity")).where(book::id, Query.eq(id)).sync();
		...
		session.update().append(book::authors, new AuthorImpl("Append", "AppendCity")).where(book::id, Query.eq(id)).sync();
